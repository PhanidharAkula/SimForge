#!/usr/bin/env python3
"""
Generate a canonical scenario bundle for New York City (500K trips).

City:   New York City, NY — Manhattan and surrounding area
Trips:  500,000 OD pairs
Scope:  ~15 km radius covering NYC boroughs (Manhattan, Brooklyn, Queens, Bronx)
...
Usage:
  python scripts/generate_nyc_500k.py                              # synthetic demand
  python scripts/generate_nyc_500k.py --model nyc_model.txt  # census demand
  python scripts/generate_nyc_500k.py --model nyc_model.txt --demand-only

Output: scenarios/nyc_500k/
"""
...
# ---------------------------------------------------------------------------
CITY_KEY        = "nyc"
SCENARIO_ID     = "nyc_500k"
DESCRIPTION     = "New York City — 500K trip tier"

CENTER_LAT      = 40.758
CENTER_LON      = -73.9855
RADIUS_KM       = 15.0          # multi-borough area for 500K trips

NUM_TRIPS       = 500_000
SEED            = 42
HORIZON_START   = 0
HORIZON_END     = 14400          # 4-hour AM peak window (6 AM – 10 AM)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to ModelGen file (e.g. nyc_model.txt). "
