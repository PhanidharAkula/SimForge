#!/usr/bin/env python3
"""
Generate a canonical scenario bundle for Los Angeles (5M trips).

City:   Los Angeles, CA — downtown and surrounding urban area
Trips:  5,000,000 OD pairs
Scope:  ~50 km radius covering the LA metropolitan area
...
Usage:
  python scripts/generate_la_5m.py                              # synthetic demand
  python scripts/generate_la_5m.py --model la_model.txt  # census demand
  python scripts/generate_la_5m.py --model la_model.txt --demand-only

Output: scenarios/la_5m/
"""
...
CITY_KEY        = "la"
SCENARIO_ID     = "la_5m"
DESCRIPTION     = "Los Angeles — 5M trip tier"

CENTER_LAT      = 34.0522
CENTER_LON      = -118.2437
RADIUS_KM       = 50.0          # metro-wide coverage; requires WGTP expansion above ~1M raw car trips

NUM_TRIPS       = 5_000_000
SEED            = 42
HORIZON_START   = 0
HORIZON_END     = 28800          # 8-hour workday window (6 AM – 2 PM)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to ModelGen file (e.g. la_model.txt). "
