# Canonical Routes + Parallel BFS

**Status**: implemented; cluster re-measured on Cardinal.
...
The earlier large-tier benchmark run on Cardinal (job 9332478, 9332482)
made an unmodeled cost visible: at 200K trips on the chicago bundle, the
state-aware BFS routes trips at **~1.5 s/trip**; at 500K on the nyc
bundle, **~2.16 s/trip**. Both rates are ~25× slower than the earlier
wall-time budgets assumed. The empirical projection:
...
|---|---|---|
| **Canonical routes** | Compute BFS once per scenario, share between SUMO and MATSim adapters | 2× on cold prep (eliminates the duplication) |
| **Parallel BFS** | `multiprocessing.Pool` across trips with deterministic merge | ~12-15× (16-core Amdahl, serial setup + merge ~5% of wall) |

Combined, expected `cold_prep_wall` on Cardinal:

| Scenario | Before | Canonical routes only | Canonical + parallel |
|---|---|---|---|
...
invalidate the entire cache namespace if the routing algorithm changes
...
| **C/Cython extension with shared graph + threads (releasing GIL)** | Could be 5–10× faster per worker; no graph duplication | Requires writing + maintaining native code; loses the cross-platform pure-Python guarantee; build-time complexity | ❌ not warranted yet, possible future work |
...
def _ensure_prepared_cache(self, scenario_path, scenario_id, engine, engine_options):
    ...
    # Compute canonical routes once, share across adapters
...
|---|---|---|---|
| Skeleton | Design doc + skeleton tests | 9 contract tests, all skip (module absent) | ✅ landed |
| Serial BFS | `canonical_routes.py` serial impl + JSONL cache | 7 tests pass (4 API + 2 cache + 1 byte-identity) | ✅ landed |
| SUMO adapter | SUMO adapter accepts `canonical_routes=` kwarg | `TestSumoRoutesXmlByteIdentity` passes | ✅ landed |
| MATSim adapter | MATSim adapter accepts `canonical_routes=` kwarg | `TestMatsimPlansXmlByteIdentity` passes | ✅ landed |
| Harness wiring | Harness wires shared BFS pass | 12 cache-management tests pass (stubs added) | ✅ landed |
| Parallel BFS | `multiprocessing.Pool` in `canonical_routes.py` | `TestParallelDeterminism` passes (workers=2,4) | ✅ landed |
| Documentation | Documentation pass | n/a | ✅ landed |
| Re-measurement | Cluster re-measurement on Cardinal | post-run wall numbers vs the earlier projections | ✅ done |

The full suite passes `python -m pytest tests/test_canonical_routes.py
tests/test_sumo_adapter.py tests/test_matsim_adapter.py
tests/test_adapter_determinism.py tests/test_run_benchmark.py`
(51 passed, 9 arm64-skipped, 0 failed).
...
The adapter functions keep their original signature working when
...
The original cache was scoped per-output-dir
...
Re-run the benchmark to quantify the speedup against the pre-optimization baseline:

1. **Baseline**: chicago_200k_car job 9332478 completed 2026-05-18 at
   141.87 h wall on Cardinal Xeon Max 9470 (pre-optimization reference).
2. **After the optimization (chicago_200k_car)**: job 9971041 measured ~7.14 h
   ...
   warm-cache re-run vs the baseline cold ≈ 228×.
3. **After the optimization (nyc_500k_car)**: job 9971042, BFS pass at
   ~6.7 trips/s on 24 workers projecting ~20 h cold + ~1-2 h sim ≈ ~22 h
   cold-cache total.

Both scenarios now submit via the single `cluster/jobs/benchmark_large.sbatch`
umbrella (`--time=2-00:00:00`). The earlier per-scenario fallback
sbatchs (`benchmark_large_chicago_200k.sbatch` + `benchmark_large_nyc_500k.sbatch`)
were deleted in commit 503bcbf once both scenarios were confirmed to fit
...
Numbers go into:
- `CHANGELOG.md` canonical-routes entry, in a measured-speedup table.
...
- **Should the per-engine `.cache/<engine>/` layer be deprecated
  now that canonical routes exist?** It currently caches the *full adapter
  output* (XML files), which is still useful when re-running the same engine
  with a different seed. Decision: keep it. Canonical routes sit *above* it.
...
- **Worker count default**: 16 matches the typical SBATCH `--cpus-per-task`
  ...
  there are too few trips. **Resolved by deployment:**
