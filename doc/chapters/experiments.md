...
| LPSim        | Mesoscopic  | GPU-accelerated queue model          | GPU      |

> The plan listed five engines (SUMO, MATSim, POLARIS, LPSim, QarSUMO). Per advisor agreement, Version_4 narrows scope to **three primary engines** (SUMO, MATSim, LPSim) with POLARIS and QarSUMO documented as backups. QarSUMO was dropped (Phase A) after a 2026-04-26 audit confirmed no usable public source; LPSim was integrated (Phase B). The canonical stress test now runs all 4 cells (SUMO meso, SUMO micro, MATSim meso, LPSim meso) at N=5 repeats each.
...
| --- | --- | --- | --- | --- |
| Stress test | `chicago_1k_car` | SUMO meso, SUMO micro, MATSim meso, LPSim meso | 5 each | **20 runs** |

Every `feasibility_report.json` confirms `feasible_trips == 1000` for every adapter, so the four engines simulate exactly the same trip set. The LPSim cell completes only on a CUDA-capable host; on a dev laptop without a GPU it records a clean failure with a build pointer (no silent CPU fallback).
...
The 10K – 500K tiers are generated by `scripts/02_small_commute.py`, `scripts/03_medium_multimodal.py`, `scripts/04_large_full_day.py`, and `scripts/05_stress_test.py`. They target NYC, LA, and Chicago respectively and run on OSC Pitzer — see [`doc/PITZER.md`](../PITZER.md) for the operational guide.
...
Used for the larger tiers (50K – 500K) that exceed the arm64 `netconvert` threshold and for every LPSim run (LPSim needs CUDA). All three primary engines (SUMO, MATSim, LPSim) run end-to-end on a single Pitzer GPU node — SUMO and MATSim share the CPU side, LPSim drives the V100. See `doc/PITZER.md` §7 for the full operational guide.
...
| Partitions used | `gpu` (SUMO + MATSim + LPSim share the same GPU node)            |
...
| LPSim    | git clone    | Xuan-1998/LPSim or `yibo123/lpsim:cuda12.4` SIF | GPU only (built via cluster/jobs/build_lpsim.sbatch) |
