...
**Variable 1: Simulator Engine (3 configurations evaluated; 4 once LPSim lands)**
...
| LPSim (planned) | Mesoscopic  | GPU-accelerated queue model          | GPU      |

> The plan listed five engines (SUMO, MATSim, POLARIS, LPSim, QarSUMO). Per advisor agreement, Version_4 narrows scope to **three primary engines** (SUMO, MATSim, LPSim) with POLARIS and QarSUMO documented as backups. QarSUMO was dropped after a 2026-04-26 audit confirmed no usable public source. LPSim is scheduled for Version_4 Phase B (`todo.md`); the canonical stress test below currently runs the **3-cell** subset (SUMO meso, SUMO micro, MATSim meso).
...
| --- | --- | --- | --- | --- |
| Stress test | `chicago_1k_car` | SUMO meso, SUMO micro, MATSim meso | 3 (MATSim 2) | **8 runs** |

Total wall-clock on Apple M4 Pro: ~22 s. Every cell completes successfully and every `feasibility_report.json` confirms `feasible_trips == 1000`. Once LPSim lands in Version_4 Phase B, this matrix grows to 4 cells / 11 runs.
...
Used for the larger tiers (50K – 500K) that exceed the arm64 `netconvert` threshold; the GPU partition is reserved for LPSim once Phase B lands. SUMO and MATSim run on the cpu partition today; see `doc/SCENARIO_GENERATION.md` §9 for the operational guide.
...
| Partitions used | `cpu` (SUMO + MATSim today; LPSim will switch to `gpu` / `gpu-quad` once integrated) |
...
| LPSim    | (planned)    | `yibo123/lpsim:cuda12.4` Docker image | Version_4 Phase B (GPU only) |
