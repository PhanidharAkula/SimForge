...
3. **No claim about 200 K + tier behaviour.** The bundled `benchmark_small.yaml` runspec covers up to la_50k_car. The 200 K and 500 K tiers are generated by `scripts/04_chicago_200k_car.py` and `scripts/05_nyc_500k_car.py` and would require a separate, longer-walltime SLURM job. The DTALite scalability ceiling identified at 50 K applies a fortiori at 200 K + and would require the same path4gmns mitigation before those tiers could be reported.
