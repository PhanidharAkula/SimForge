...
| —                     | `zone_id`       | Set to `node_id` IFF node appears as origin/dest in demand; otherwise blank (transit-only) |
| —                     | `production`    | 1 if zone, 0 otherwise                          |
| —                     | `attraction`    | 1 if zone, 0 otherwise                          |
...
| —                           | `capacity` (vph) | lanes × 1800 (FHWA HCM urban-arterial)  |
| —                           | `link_type`      | 1 (Highway/Expressway)                  |
| —                           | `facility_type`  | "Highway"                               |
| —                           | `dir_flag`       | 1 (one-way; canonical links already directional) |
| —                           | `VDF_fftt1` (min)| length_km / free_speed_kmh × 60         |
| —                           | `VDF_cap1`       | = capacity                              |
| —                           | `VDF_alpha1`     | 0.15 (FHWA BPR default)                 |
| —                           | `VDF_beta1`      | 4   (FHWA BPR default)                  |
...
| —                          | `volume`         | Aggregated count: number of trips per (origin, destination) pair |
...
- **SimForge engine retrospectives:** `doc/engines/{LPSIM,QARSUMO}_RETROSPECTIVE.md`, `doc/engines/THIRD_ENGINE_OPTIONS.md`
